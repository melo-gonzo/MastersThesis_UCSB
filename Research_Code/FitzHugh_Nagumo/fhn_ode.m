function [t,tp,xsol,xsolp]=fhn_ode(p)
tp=[];
tspan=p.tspan;
if p.perturbed~=0
...
if length(p.a)>1 || length(p.b)>1 || length(p.c1)>1 || length(p.c2)>1 || length(p.d)>1
    if isfield(p,'tpert')
        if ~isfield(p,'tau')
            p.tau=1;
        end 
        ...
        p.tau=1;
        p.tpert=0:p.tau:max([length(p.a),length(p.b),...
            length(p.c1),length(p.c2),length(p.d)])-1;
        p.tpert=[0 p.tpert];
        p.tpert(end)=p.tspan(end);
    end
    segs=ones(5,length(p.tpert)-1);
    segs(1,:)=p.a.*segs(1,:);
    segs(2,:)=p.b.*segs(2,:);
    segs(3,:)=p.c1.*segs(3,:);
    segs(4,:)=p.c2.*segs(4,:);
...
p.d=segs(5,1);
if length(p.tspan)>1
[t,xsol]=ode45(@(t,x) fhn_ode_equations(t,x,p),p.tspan,p.x0,options);
else
    [t,xsol]=deal(nan);
end 
tidxhit1=[];
...
if length(p.tpert)<1
    ...
    for k=2:length(p.tpert)
        ...
        if length(p.tspan)>1
            [tseg,xseg]=ode45(@(t,x) fhn_ode_equations(t,x,p),p.tspan,p.x0,options);
            tp=[tp;tseg];
            xsolp=[xsolp;xseg];
