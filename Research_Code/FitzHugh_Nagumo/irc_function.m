...
if numeric==1
    ...
    %
    while sum(abs(irc(1,:)-irc(end,:))>10^-5)>0 && k<100
        if k>1
            c0=([1;1]*c0');
        end
        ...
        for n=1:2
            ...
            irc_pos=irc_pos/(irc_pos(1,:)*V(:,scale_vector_idx));
            irc_neg=irc_neg/(irc_neg(1,:)*V(:,scale_vector_idx));
